...
exports.SpaClient = exports.FLOW_STATES = exports.FLOW_FAILED = exports.FLOW_LOW = exports.FLOW_GOOD = void 0;
const crc = __importStar(require("crc"));
const net = __importStar(require("net"));
exports.FLOW_GOOD = "Good";
exports.FLOW_LOW = "Low";
exports.FLOW_FAILED = "Failed";
exports.FLOW_STATES = [exports.FLOW_GOOD, exports.FLOW_LOW, exports.FLOW_FAILED];
...
class SpaClient {
    constructor(log, host, spaConfigurationKnownCallback, changesCallback, reconnectedCallback, devMode) {
        this.log = log;
        this.host = host;
        this.spaConfigurationKnownCallback = spaConfigurationKnownCallback;
        this.changesCallback = changesCallback;
        this.reconnectedCallback = reconnectedCallback;
        this.autoSetSpaClock = true;
        // Reconnect backoff state
        this.reconnectDelay = RECONNECT_DELAY_MIN_MS;
        this.numberOfConnectionsSoFar = 0;
        // Firmware/model info logged on first connect
        this.firmwareVersion = 'unknown';
        this.systemModel = 'unknown';
    ...
    get_socket(host) {
        ...
        this.socket = net.connect({
            ...
            this.log.info('Successfully connected to Spa at', host, 'on port 4257.' + modelInfo, 'Connection #' + this.numberOfConnectionsSoFar, 'in', diffDays, 'days');
        ...
        (_c = this.socket) === null || _c === void 0 ? void 0 : _c.on('error', (error) => {
     ...
     * @param chunk
     */
    readAndActOnSocketContents(chunk) {
        ...
        while (chunk.length > 0) {
            ...
            if (msgLength > (chunk.length - 2)) {
                ...
                this.log.debug("Incomplete message received (awaiting more data)", this.prettify(chunk), "missing", (msgLength - chunk.length + 2), "bytes");
                break;
            }
    ...
    checkWeHaveReceivedStateUpdate() {
        if (this.receivedStateUpdate) {
            ...
            this.log.warn('No spa state update received (missed checks:', this.missedStateChecks + ').', 'Last state was', this.stateToString());
     ...
     * @param payload
     */
    sendMessageToSpa(purpose, type, payload) {
        var _a;
    ...
    getIsLocked(entirePanel) {
        return entirePanel ? this.lockTheEntirePanel : this.lockTheSettings;
    }
    setIsLocked(entirePanel, value) {
        if (this.getIsLocked(entirePanel) == value)
            return;
     ...
     * to avoid freezing.
     */
    setHeatingModeAlwaysReady(isAlwaysReady) {
    ...
    static getSpeedAsString(range, speed) {
        if (range == 1) {
     ...
     */
    setPumpSpeed(index, desiredSpeed) {
     ...
     */
    send_toggle_message(itemName, code) {
        if (code > 255) {
        ...
        // outcome).
        this.sendMessageToSpa("Toggle " + itemName + ", using code:" + code, ToggleItemRequest, new Uint8Array([code, 0x00]));
    }
    ...
    send_lock_settings(entirePanel, lock) {
    ...
    convertSpaTemperatureToExternal(temperature) {
        if (this.temp_CorF === FAHRENHEIT)
            return temperature;
    ...
    // So '80' stored/sent by the Spa means 40.0 degrees celsius, IF the spa
    // is operating in Celsius. If in Fahrenheit then no conversions needed.
    convertExternalTemperatureToSpa(temperature) {
        if (this.temp_CorF === FAHRENHEIT)
            return temperature;
        return Math.round(temperature * 2.0);
    }
    internalTemperatureToString(temperature) {
     ...
     */
    readAndActOnMessage(length, checksum, chunk) {
        if (chunk[0] != 0x7e || chunk[1] != length || chunk[length] != checksum || chunk[length + 1] != 0x7e || chunk.length != (length + 2)) {
            this.log.error("Bad internal data in message handling. Please report a bug:", length, checksum, this.prettify(chunk));
            return false;
        }
        const contents = chunk.slice(5, length);
        const msgType = chunk.slice(2, 5);
        let stateChanged;
        let avoidHighFreqDevMessage = true;
        if (this.equal(msgType, StateReply)) {
            ...
            if (!recognised) {
                this.log.info("Not understood a received spa message", "(nothing critical, but please do report this):" + this.prettify(msgType), " contents: " + this.prettify(contents));
            }
        }
    ...
    readStateFromBytes(bytes) {
        ...
        this.pumpsCurrentSpeed[0] = this.internalSetPumpSpeed(this.pumpsSpeedRange[0], (pump_status1234 & (1 + 2)));
        this.pumpsCurrentSpeed[1] = this.internalSetPumpSpeed(this.pumpsSpeedRange[1], (pump_status1234 & (4 + 8)) >> 2);
        this.pumpsCurrentSpeed[2] = this.internalSetPumpSpeed(this.pumpsSpeedRange[2], (pump_status1234 & (16 + 32)) >> 4);
        this.pumpsCurrentSpeed[3] = this.internalSetPumpSpeed(this.pumpsSpeedRange[3], (pump_status1234 & (64 + 128)) >> 6);
        // pumps 5,6 are untested by me.
        const pump_status56 = bytes[12];
        this.pumpsCurrentSpeed[4] = this.internalSetPumpSpeed(this.pumpsSpeedRange[4], (pump_status56 & (1 + 2)));
        this.pumpsCurrentSpeed[5] = this.internalSetPumpSpeed(this.pumpsSpeedRange[5], (pump_status56 & (4 + 8)) >> 2);
        ...
        if (this.lightIsOn[0] != undefined)
            this.lightIsOn[0] = ((bytes[14] & (1 + 2)) === (1 + 2));
        if (this.lightIsOn[1] != undefined)
        ...
        // The two aux devices:
        if (this.auxIsOn[0] != undefined)
            this.auxIsOn[0] = (bytes[15] & 0x08) !== 0;
        if (this.auxIsOn[1] != undefined)
            this.auxIsOn[1] = (bytes[15] & 0x10) !== 0;
        ...
        if (oldBytes.length != this.lastStateBytes.length)
            return true;
    ...
    internalSetPumpSpeed(range, value) {
        if (range === 1) {
     ...
     */
    interpretControlTypesReply(bytes) {
        if (this.accurateConfigReadFromSpa) {
        ...
        this.log.info("Discovered other components: circulation-pump", this.hasCirculationPump, ", blower", this.blowerSpeedRange, ", mister", this.misterIsOn, ", aux", aux);
     ...
     * @param contents
     */
    interpretControlPanelReply(id, contents) {
        ...
        if (id == 1) {
            ...
            this.log.info("Second filter time", (filter2on ? 'on' : 'off'), "from", filter2start, "for", filter2duration);
        }
        else if (id == 2) {
            ...
            let motherboard = "";
            contents.slice(4, 12).forEach((byte) => {
    ...
    readFaults(bytes) {
        ...
        if (!this.equal(oldBytes, this.lastFaultBytes)) {
            this.log.info(message, daysAgo, "days ago of type", "M0" + code, "=", this.faultCodeToString(code), "with details from log:", "Fault Entries:", bytes[0], ", Num:", bytes[1] + 1, ", Error code:", "M0" + code, ", Days ago:", daysAgo, ", Time:", this.timeToString(hour, minute), ", Heat mode:", bytes[6], ", Set temp:", this.internalTemperatureToString(bytes[7]), ", Temp A:", this.internalTemperatureToString(bytes[8]), ", Temp B:", this.internalTemperatureToString(bytes[9]));
        }
        return stateChanged;
    }
    equal(one, two) {
    ...
    faultCodeToString(code) {
        if (code == 15)
            return "sensors may be out of sync";
        if (code == 16)
            return "the water flow is low";
        if (code == 17)
            return "the water flow has failed";
        if (code == 19)
            return "priming (this is not actually a fault - your Spa was recently turned on)";
        if (code == 20)
            return "the clock has failed";
        if (code == 21)
            return "the settings have been reset (persistent memory error)";
        if (code == 22)
            return "program memory failure";
        if (code == 26)
            return "sensors are out of sync -- call for service";
        if (code == 27)
            return "the heater is dry";
        if (code == 28)
            return "the heater may be dry";
        if (code == 29)
            return "the water is too hot";
        if (code == 30)
            return "the heater is too hot";
        if (code == 31)
            return "sensor A fault";
        if (code == 32)
            return "sensor B fault";
        if (code == 33)
            return "safety trip - pump suction blockage";
        if (code == 34)
            return "a pump may be stuck on";
        if (code == 35)
            return "hot fault";
        if (code == 36)
            return "the GFCI test failed";
        if (code == 37)
            return "hold mode activated (this is not actually a fault)";
        return "unknown code - check Balboa spa manuals";
    }
}
exports.SpaClient = SpaClient;
//# sourceMappingURL=spaClient.js.map