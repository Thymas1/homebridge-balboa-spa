...
class ThermostatAccessory {
    constructor(platform, accessory) {
        ...
        this.service = (_a = this.accessory.getService(this.platform.Service.Thermostat)) !== null && _a !== void 0 ? _a : this.accessory.addService(this.platform.Service.Thermostat);
        ...
        this.service.getCharacteristic(this.platform.Characteristic.CurrentTemperature)
            .on("get" /* CharacteristicEventTypes.GET */, this.getCurrentTemperature.bind(this));
        ...
        this.service.getCharacteristic(this.platform.Characteristic.TemperatureDisplayUnits)
            .on("get" /* CharacteristicEventTypes.GET */, this.getTemperatureDisplayUnits.bind(this));
        this.service.getCharacteristic(this.platform.Characteristic.CurrentHeatingCoolingState)
            .on("get" /* CharacteristicEventTypes.GET */, this.getHeatingState.bind(this));
        // Adjust properties to only allow Off and Heat (not Cool or Auto which are irrelevant)
        this.service.getCharacteristic(this.platform.Characteristic.TargetHeatingCoolingState)
            .on("set" /* CharacteristicEventTypes.SET */, this.setTargetHeatingState.bind(this)).setProps({
            minValue: 0,
            maxValue: 2,
            validValues: [0, 1, 2]
        })
            .on("get" /* CharacteristicEventTypes.GET */, this.getTargetHeatingState.bind(this));
    }
    ...
    getTargetHeatingState(callback) {
        if (!this.platform.isCurrentlyConnected()) {
            callback(this.platform.connectionProblem);
        }
        else {
            ...
            const flowError = (this.platform.spa.getFlowState() == spaClient_1.FLOW_FAILED);
            let result;
            if (flowError) {
            ...
            this.platform.log.debug('Get Target Heating State <-', mode ? "HEAT" : "COOL", ", Flow error(" + flowError + ")", result, this.platform.status());
            callback(null, result);
        }
    }
    setTargetHeatingState(value, callback) {
        if (!this.platform.isCurrentlyConnected()) {
        ...
        if (value == this.platform.Characteristic.TargetHeatingCoolingState.OFF) {
            ...
            if (this.platform.spa.getFlowState() == spaClient_1.FLOW_GOOD) {
        ...
        else if (this.platform.spa.getFlowState() != spaClient_1.FLOW_GOOD) {
        ...
        this.platform.log.debug('Set Target Heating State ->', heating ? "HEAT" : "COOL", value, "(and need to adjust valid range)", this.platform.status());
    ...
    setTargetTemperature(value, callback) {
        ...
        if (this.platform.spa.getTempRangeIsHigh()) {
            if (temp < 26.5) {
                ...
                this.service.getCharacteristic(this.platform.Characteristic.TargetTemperature)
                    .updateValue(temp);
                callback(new Error("Temperature " + value + " out of bounds [26.5,40.0]; using " + temp));
                return;
            }
        }
        else {
            if (temp > 36.0) {
                ...
                this.service.getCharacteristic(this.platform.Characteristic.TargetTemperature)
                    .updateValue(temp);
                callback(new Error("Temperature " + value + " out of bounds [10.0,36.0]; using " + temp));
                return;
            }
        }
    ...
    updateCharacteristics() {
        ...
        this.platform.log.debug('Thermostat updating to: target:', targetTemperature, '(=' + targetTempVal + 'C)', ', current:', temperature, '(=' + tempVal + 'C), is high:', mode, ', is heating:', heating, ', flow state:', flowState);
        ...
        this.service.getCharacteristic(this.platform.Characteristic.TargetHeatingCoolingState).updateValue(flowState != spaClient_1.FLOW_GOOD ? this.platform.Characteristic.TargetHeatingCoolingState.OFF :
        ...
        this.service.getCharacteristic(this.platform.Characteristic.CurrentHeatingCoolingState).updateValue(heating ? this.platform.Characteristic.CurrentHeatingCoolingState.HEAT
            : this.platform.Characteristic.CurrentHeatingCoolingState.OFF);
    }
}
